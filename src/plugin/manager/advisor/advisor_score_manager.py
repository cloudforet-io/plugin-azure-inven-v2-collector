...
class ScoreManager(AzureBaseManager):
    ...
    def create_cloud_service(self, options: dict, secret_data: dict, schema: str):
        ...
        scores: list = advisor_conn.list_scores(secret_data["subscription_id"]).get("value", []) or []
            ...
            try:
                ...
                for time_series_info in time_series_infos:
                    ...
                    if aggregation_level == "Daily":
                        daily_score_history_display = time_series_info.get("scoreHistory")
                        score_info["daily_score_history_display"] = daily_score_history_display
                    elif aggregation_level == "Weekly":
                        weekly_score_history_display = time_series_info.get("scoreHistory")
                        score_info["weekly_score_history_display"] = weekly_score_history_display
                    elif aggregation_level == "Monthly":
                        monthly_score_history_display = time_series_info.get("scoreHistory")
                        score_info["monthly_score_history_display"] = monthly_score_history_display
                ...
                _LOGGER.error(f"[create_cloud_service] Error {self.cloud_service_type} {e}", exc_info=True)
                _LOGGER.error(f"[create_cloud_service] Error {self.cloud_service_type} {score_value}", exc_info=True)
                _LOGGER.error(f"[create_cloud_service] Error {self.cloud_service_type} {scores}", exc_info=True)
    ...
    def _get_score_category_from_id(score_id: id) -> Union[str, None]:
        ...
        if score_category in ["Cost", "Security", "Performance", "OperationalExcellence", "HighAvailability",
                              "Advisor"]:
