...
from plugin.connector.service_health.service_health_connector import ServiceHealthConnector
...
class HealthHistoryManager(AzureBaseManager):
    ...
    def create_cloud_service(self, options: dict, secret_data: dict, schema: str):
        ...
        external_link_format = "https://app.azure.com/h/{resource_id}/" + secret_data["subscription_id"][
                                                                          :3] + secret_data["subscription_id"][-3:]
            ...
            try:
                ...
                for impact in health_history_info.get("impact", []):
                    ...
                    for impacted_region in impacted_regions:
                        ...
                        impacted_regions_display.append(impacted_region.get("impacted_region"))
                        impacted_subscriptions_display.extend(impacted_region.get("impacted_subscriptions", []))

                        updates = impacted_region.get("updates", [])
                        impact_updates = self._create_impact_updates_display(updates, impacted_service, impacted_region)
                        if impact_updates:
                ...
                if impacted_services_display:
                    health_history_info["impacted_services_display"] = list(set(impacted_services_display))

                if impacted_subscriptions_display:
                    health_history_info["impacted_subscriptions_display"] = list(set(impacted_subscriptions_display))

                if impacted_regions_display:
                    health_history_info["impacted_regions_display"] = list(set(impacted_regions_display))

                if impact_updates_display:
                    health_history_info["impact_updates_display"] = impact_updates_display
                ...
                cloud_services.append(
                    make_cloud_service(
                        ...
                        reference=self.make_reference(health_history_info.get("id"), external_link_format)
                    )
                )
    ...
    def create_cloud_service_type(self):
        return make_cloud_service_type(
            ...
            labels=["Management"],
            tags={
                "spaceone:icon": f"{ICON_URL}/azure-service-health.svg"
            }
        )
